...
const reactShapePlugin = {
    ...
    run({ fileName, sourceFile, options, text }) {
        ...
        const insertPropTypesRequireableNode = () => {
            if (shouldAddPropTypesImport) {
                updates.push({
                    kind: 'insert',
                    index: 0,
                    text: `${printer.printNode(ts.EmitHint.Unspecified, getPropTypesImportNode(), sourceFile)}\n`,
                });
        ...
        const splitVariableExport = (node, shapeName) => {
            ...
            const newExport = ts.createExportDeclaration(undefined, undefined, ts.createNamedExports([
        ...
        const typesAndInterfaces = sourceFile.statements.filter((node) => ts.isInterfaceDeclaration(node) || ts.isTypeAliasDeclaration(node));
        for (const node of sourceFile.statements) {
                ...
                if (variableDeclaration && variableDeclaration.initializer && !variableDeclaration.type) {
                    ...
                    if (ts.isCallExpression(variableDeclaration.initializer) &&
                        ...
                        if (!typesAndInterfaces.find((tNode) => tNode.name.text === variableDeclaration.name.getText())) {
                            updates.push({
                                kind: 'insert',
                                index: node.pos,
                                text: `\n\n${printer.printNode(ts.EmitHint.Unspecified, getTypeForTheShape(shapeNode, shapeName, sourceFile, options), sourceFile)}`,
                            });
                        }
                        const updatedVariableDeclaration = ts.updateVariableDeclaration(variableDeclaration, variableDeclaration.name, getShapeTypeNode(shapeName), variableDeclaration.initializer);
                        const index = variableDeclaration.pos + 1;
                        const length = variableDeclaration.end - index;
                        const text = printer.printNode(ts.EmitHint.Unspecified, updatedVariableDeclaration, sourceFile);
                    ...
                    if (ts.isCallExpression(variableDeclaration.initializer) &&
                        ...
                        updates.push({
                            kind: 'insert',
                            index: node.pos,
                            text: `\n\n${printer.printNode(ts.EmitHint.Unspecified, getTypeForTheShape(shapeNode, shapeName, sourceFile, options, true), sourceFile)}`,
                        });
                        if (exportModifier) {
                            splitVariableExport(node, shapeName);
                        }
            ...
            if (ts.isExportAssignment(node) &&
                ...
                updates.push({
                    ...
                    index: node.pos,
                    length: node.end,
                    text: `${ts.sys.newLine}${printer.printNode(ts.EmitHint.Unspecified, ts.createVariableStatement([], ts.createVariableDeclarationList([ts.createVariableDeclaration(shapeName, getShapeTypeNode(shapeName), shapeNode)], ts.NodeFlags.Const)), sourceFile)}`,
                });
                const exportShapeExpression = `${ts.sys.newLine}${printer.printNode(ts.EmitHint.Unspecified, ts.createExportAssignment(undefined, undefined, undefined, ts.createIdentifier(shapeName)), sourceFile)}`;
                updates.push({
                    kind: 'insert',
                    index: node.end,
...
function getTypeForTheShape(shapeNode, shapeName, sourceFile, options, isArrayShapeType = false) {
    const shapeTypeVariable = getTypeFromPropTypesObjectLiteral(shapeNode.arguments[0], sourceFile, {
        anyAlias: options.anyAlias,
        anyFunctionAlias: options.anyFunctionAlias,
        spreadReplacements: [],
    });
    const propsTypeAlias = ts.createTypeAliasDeclaration(undefined, undefined, shapeName, undefined, isArrayShapeType ? ts.createArrayTypeNode(shapeTypeVariable) : shapeTypeVariable);
...
function getPropTypesImportNode() {
    return ts.createImportDeclaration(undefined, undefined, ts.createImportClause(ts.createIdentifier('PropTypes'), undefined), ts.createStringLiteral('prop-types'));
}
// @TODO: PropTypes.Requireable<ShapeType> doesn't works with react-validators Shapes
function getShapeTypeNode(shapeName) {
    return ts.createTypeReferenceNode(ts.createQualifiedName(ts.createIdentifier('PropTypes'), ts.createIdentifier('Requireable')), [ts.createTypeReferenceNode(ts.createIdentifier(shapeName), undefined)]);
}
