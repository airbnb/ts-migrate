...
const reactDefaultPropsPlugin = {
    ...
    run({ sourceFile, text, options }) {
        ...
        const sfcsDefaultPropsAssignments = expressionStatements.filter((expressionStatement) => ts.isBinaryExpression(expressionStatement.expression) &&
        ...
        const insertWithDefaultPropsImport = () => {
            if (shouldAddWithDefaultPropsImport) {
                updates.push({
                    kind: 'insert',
                    index: 0,
                    text: `${printer.printNode(ts.EmitHint.Unspecified, getWithDefaultPropsImport(), sourceFile)}\n`,
                });
                // it probably could be done in the better way :)
                shouldAddWithDefaultPropsImport = false;
            }
        };
        const modifyAndInsertPropsType = (propsTypeAliasDeclaration, defaultPropsTypeName, propsTypeName, newTypeInsertPos, componentTypeReference) => {
            ...
            const alreadyHaveDefalutProps = ts.isIntersectionTypeNode(propsTypeAliasDeclaration.type)
                ? propsTypeAliasDeclaration.type.types.some((typeExp) => (ts.isTypeQueryNode(typeExp) &&
            ...
            if (alreadyHaveDefalutProps)
                return;
            if (!options.skipUsingDefaultPropsHelper)
            ...
            const indexOfTypeValue = ts.isIntersectionTypeNode(propsTypeAliasDeclaration.type)
                ...
                : -1;
            const propTypesAreOnlyReferences = ts.isIntersectionTypeNode(propsTypeAliasDeclaration.type) && indexOfTypeValue === -1;
            const propsTypeValueNode = ts.isIntersectionTypeNode(propsTypeAliasDeclaration.type)
                ? ts.updateIntersectionTypeNode(propsTypeAliasDeclaration.type, ts.createNodeArray(propsTypeAliasDeclaration.type.types.filter((_, k) => propTypesAreOnlyReferences || indexOfTypeValue === k)))
                : propsTypeAliasDeclaration.type;
            const doesPropsTypeHaveExport = propsTypeAliasDeclaration.modifiers &&
                propsTypeAliasDeclaration.modifiers.find((modifier) => modifier.kind === ts.SyntaxKind.ExportKeyword);
            // rename type PropName -> type OwnPropName
            const updatedPropTypesName = doesPropsTypeHaveExport ? propsTypeName : `Own${propsTypeName}`;
            const updatedPropTypeAlias = ts.updateTypeAliasDeclaration(propsTypeAliasDeclaration, propsTypeAliasDeclaration.decorators, propsTypeAliasDeclaration.modifiers, ts.createIdentifier(updatedPropTypesName), propsTypeAliasDeclaration.typeParameters, propsTypeValueNode);
            ...
            const componentPropsTypeName = doesPropsTypeHaveExport
                ...
                : propsTypeName;
            const newPropsTypeAlias = ts.createTypeAliasDeclaration(undefined, undefined, ts.createIdentifier(componentPropsTypeName), undefined, ts.isIntersectionTypeNode(propsTypeAliasDeclaration.type)
                ? ts.createIntersectionTypeNode([
                    newPropsTypeValue,
            ...
            if (doesPropsTypeHaveExport) {
                const updatedComponentTypeReference = ts.updateTypeReferenceNode(componentTypeReference, ts.createIdentifier(componentPropsTypeName), undefined);
                const index = componentTypeReference.pos;
                const length = componentTypeReference.end - index;
                const text = printer.printNode(ts.EmitHint.Unspecified, updatedComponentTypeReference, sourceFile);
        ...
        sfcsDefaultPropsAssignments.forEach((defaultProp) => {
            ...
            if (componentDeclaration &&
                ...
                if (propsTypeAliasDeclarations) {
                    ...
                    modifyAndInsertPropsType(propsTypeAliasDeclarations, defaultPropsTypeName, propsTypeName, newTypeInsertPos, componentDeclaration.parameters[0].type);
                }
            }
        });
        classDefaultPropsAssignment.forEach((classDeclaration) => {
            ...
            if (!defaultPropsDeclaration)
                return;
            const isObjectIdentifier = defaultPropsDeclaration.initializer && ts.isIdentifier(defaultPropsDeclaration.initializer);
            ...
            const expressionWithTypeArguments = heritageClause && heritageClause.types
                ? heritageClause.types.find((x) => ts.isExpressionWithTypeArguments(x) &&
            ...
            const propsTypeReferenceNode = expressionWithTypeArguments && expressionWithTypeArguments.typeArguments
                ? expressionWithTypeArguments.typeArguments.find((genericArgs) => ts.isTypeReferenceNode(genericArgs))
                : undefined;
            ...
            if (propsTypeAliasDeclarations) {
                ...
                modifyAndInsertPropsType(propsTypeAliasDeclarations, defaultPropsTypeName, propsTypeName, newTypeInsertPos, propsTypeReferenceNode);
            }
        });
