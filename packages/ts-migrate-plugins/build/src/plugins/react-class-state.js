...
import { isReactClassComponent, getReactComponentHeritageType, getNumComponentsInSourceFile, } from './utils/react';
    ...
    async run({ fileName, sourceFile, options }) {
        ...
        reactClassDeclarations.forEach((classDeclaration) => {
            ...
            if (!stateType && usesState(classDeclaration)) {
                ...
                const newStateType = ts.createTypeAliasDeclaration(undefined, undefined, stateTypeName, undefined, anyType);
                updates.push({
                    kind: 'insert',
                ...
                updates.push({
                    ...
                    text: ` ${printer.printNode(ts.EmitHint.Unspecified, ts.updateExpressionWithTypeArguments(heritageType, [
                        propsType || ts.createTypeLiteralNode([]),
                        ts.createTypeReferenceNode(stateTypeName, undefined),
                    ], heritageType.expression), sourceFile)}`,
                });
            }
        });
