...
export default function getTypeFromPropTypesObjectLiteral(objectLiteral, sourceFile, params) {
    const members = [];
...
function convertPropertyAssignment(propertyAssignment, sourceFile, params) {
    ...
    let propertySignature = ts.createPropertySignature(undefined, name, isRequired ? undefined : ts.createToken(ts.SyntaxKind.QuestionToken), typeNode, undefined);
...
function getTypeFromPropTypeExpression(node, sourceFile, params) {
    ...
    if (ts.isPropertyAccessExpression(node)) {
        ...
        if (/string/.test(text)) {
            ...
            if (anyFunctionAlias) {
                ...
                result = ts.createFunctionTypeNode(undefined, [
                    ts.createParameter(undefined, undefined, ts.createToken(ts.SyntaxKind.DotDotDotToken), 'args', undefined, ts.createArrayTypeNode(ts.createTypeReferenceNode(anyAlias, undefined)), undefined),
                ], ts.createTypeReferenceNode(anyAlias, undefined));
            }
            else {
                result = ts.createFunctionTypeNode(undefined, [
                    ts.createParameter(undefined, undefined, ts.createToken(ts.SyntaxKind.DotDotDotToken), 'args', undefined, ts.createArrayTypeNode(ts.createKeywordTypeNode(ts.SyntaxKind.AnyKeyword)), undefined),
        ...
        if (/oneOf$/.test(expressionText)) {
            ...
            if (ts.isArrayLiteralExpression(argument)) {
                if (argument.elements.every((elm) => ts.isStringLiteral(elm) || ts.isNumericLiteral(elm))) {
                    result = ts.createUnionTypeNode(argument.elements.map((elm) => ts.createLiteralTypeNode(elm)));
                }
            }
        }
            ...
            if (ts.isArrayLiteralExpression(argument)) {
                const children = [];
                result = ts.createUnionOrIntersectionTypeNode(ts.SyntaxKind.UnionType, argument.elements.map((elm) => {
    ...
    if (!result) {
        ...
        result = ts.addSyntheticTrailingComment(result, ts.SyntaxKind.SingleLineCommentTrivia, ` TODO: ${text
            .split('\n')
            .map((line) => line.trim())
            .join(' ')}`, true);
    }
...
export function createPropsTypeNameGetter(sourceFile) {
