...
function withExplicitAny(text, diagnostics, anyAlias) {
    ...
    replaceTS7006(root, diagnostics.filter((diagnostic) => diagnostic.code === 7006), typeAnnotation);
...
function replaceTS7006(root, diagnostics, typeAnnotation) {
    diagnostics.forEach((diagnostic) => {
        root
            ...
            if (j.ArrowFunctionExpression.check(path.parent.node) &&
                path.parent.node.params.length === 1) {
                const paramIndex = path.parent.node.params.indexOf(path.node);
                path.parent.replace(j.arrowFunctionExpression.from(Object.assign(Object.assign({}, path.parent.node), { params: [
                        ...path.parent.node.params.slice(0, paramIndex),
                        j.identifier.from(Object.assign(Object.assign({}, path.parent.node.params[paramIndex]), { typeAnnotation })),
